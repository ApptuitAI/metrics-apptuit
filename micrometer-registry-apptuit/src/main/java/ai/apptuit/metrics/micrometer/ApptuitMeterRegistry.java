...
public class ApptuitMeterRegistry extends StepMeterRegistry {
  private static final ThreadFactory DEFAULT_THREAD_FACTORY =
          new NamedThreadFactory("apptuit-metrics-publisher");

  private final Logger logger = LoggerFactory.getLogger(ApptuitMeterRegistry.class);

  private final ApptuitConfig config;

  private final Map<String, String> globalTagMap = new HashMap<String, String>();
  ...
  private ApptuitMeterRegistry(ApptuitConfig config, Clock clock, ThreadFactory threadFactory) {
    ...
    this.config = config;
    putClient = getClient();
    globalTagMap.put("process", "spring-boot-application");
    start(threadFactory);
  }
  ...
  private final ApptuitPutClient getClient() {
    return  new ApptuitPutClient(config.token(), globalTagMap);
  }
  ...
  protected void publish() {
    for (List<Meter> batch : MeterPartition.partition(this, config.batchSize())) {
      DataPointCollector collector =
              new DataPointCollector(System.currentTimeMillis() / 1000);
      for (int i = 0;i < batch.size();i++) {
        if (batch.get(i) instanceof Gauge) {
          ...
          collector.collectHistogram(batch.get(i).getId().getName(),
                  (DistributionSummary)batch.get(i));
          ...
          collector.collectLongTaskTimer(batch.get(i).getId().getName(),
                  (LongTaskTimer)batch.get(i));
        } else if (batch.get(i) instanceof FunctionTimer) {
          collector.collectFunctionTimer(batch.get(i).getId().getName(),
                  (FunctionTimer)batch.get(i));
        } else if (batch.get(i) instanceof FunctionCounter) {
          collector.collectFunctionCounter(batch.get(i).getId().getName(),
                  (FunctionCounter)batch.get(i));
  ...
  protected class DataPointCollector {
    ...
    protected void collectGauge(String name, Gauge gauge) {
      ...
      if (Double.isFinite(value)) {
        addDataPoint(addBaseUnitToName(name,gauge),
                ((Number) value).doubleValue(),gauge.getId().getTags());
      }
    }
    ...
    protected void collectLongTaskTimer(String name, LongTaskTimer longTimer) {
      addDataPoint(name + ".activeTasks",(double)longTimer.activeTasks(),
              longTimer.getId().getTags());
      addDataPoint(name + "." + getBaseTimeUnit().toString().toLowerCase() + ".duration",
              longTimer.duration(getBaseTimeUnit()),longTimer.getId().getTags());
    }

    protected void collectFunctionTimer(String name,FunctionTimer funcTimer) {
      addDataPoint(name + "." + getBaseTimeUnit().toString().toLowerCase() + ".count",
              funcTimer.count(), funcTimer.getId().getTags());
      addDataPoint(name + "." + getBaseTimeUnit().toString().toLowerCase() + ".sum",
              funcTimer.totalTime(getBaseTimeUnit()), funcTimer.getId().getTags());
      addDataPoint(name + "." + getBaseTimeUnit().toString().toLowerCase() + ".mean",
              funcTimer.mean(getBaseTimeUnit()), funcTimer.getId().getTags());
    }

    protected void collectMeter(String name,Meter meter) {
      for (Measurement measurement : meter.measure()) {
        this.addDataPoint(name + "." + measurement.getStatistic().toString().toLowerCase(),
                measurement.getValue(), meter.getId().getTags());
      }
    }

    private void reportSnapshot(TagEncodedMetricName metric, HistogramSnapshot snapshot,
                                List<Tag> tags) {
      addDataPoint(metric.submetric(getBaseTimeUnit()
                                            .toString()
                                            .toLowerCase() + ".duration.max"),
              snapshot.max(getBaseTimeUnit()), tags);
      addDataPoint(metric.submetric(getBaseTimeUnit()
                                            .toString()
                                            .toLowerCase() + ".duration.mean"),
              snapshot.mean(getBaseTimeUnit()),tags);
      addDataPoint(metric.submetric(getBaseTimeUnit()
                                            .toString()
                                            .toLowerCase() + ".duration.count"),
              snapshot.total(),tags);
      ...
      for (ValueAtPercentile obj: percentValues) {
        addDataPoint(metric.submetric(getBaseTimeUnit()
                             ...
                             .withTags("quantile", Double.toString(obj.percentile())),
                obj.value(),tags);
      }
    }
