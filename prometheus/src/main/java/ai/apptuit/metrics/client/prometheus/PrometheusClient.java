...
public class PrometheusClient {
  ...
  public QueryResponse query(long startEpochMillis, long endEpochMillis, String promQueryString)
    ...
    String basicAuth = new String(Base64.getEncoder().encode((token + ":" + token).getBytes()));
    ...
    System.out.println("Sending 'POST' request to URL : " + queryEndpoint);
    System.out.println("Post parameters : " + urlParameters);

    InputStream inputStr = (responseCode < HttpURLConnection.HTTP_BAD_REQUEST) ? urlConnection.getInputStream() : urlConnection.getErrorStream();
  ...
  static long getStepSize(long queryRangeMillis) {
    ...
    return lowerStep * Math.round(step / lowerStep);
  }
}
